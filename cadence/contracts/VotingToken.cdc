// VotingToken.cdc
//
// The VotingToken contract is a sample implementation of a fungible token on Flow which can be used for voting.
...
pub contract VotingToken {
    ...
    pub resource interface VotingPower {
        pub let votingPowerDataSnapshot: [VotingPowerData]
    }
    ...
    // new tokens.
    //
    pub resource Vault: Provider, Receiver, Balance, VotingPower {
        ...
        init(balance: UFix64) {
            ...
            self.votingPowerDataSnapshot = []
        }

        priv fun recordVotingPower() {
            let ts = getCurrentBlock().timestamp
            self.votingPowerDataSnapshot.append(VotingPowerData(vaultBalance: self.balance, blockTs: ts))
        }
    ...
    pub resource VaultMinter {
        ...
        pub fun mintTokens(amount: UFix64, recipient: Capability<&AnyResource{Receiver}>) {
            ...
            VotingToken.totalSupply = VotingToken.totalSupply + UFix64(amount)
    ...
    init() {
        ...
        // assign paths
        self.VaultStoragePath = /storage/CadenceVotingTokenVault
        self.MinterStoragePath = /storage/CadenceVotingTokenMinter
        self.VaultPublicPath = /public/CadenceVotingTokenVault
        self.MinterPrivatePath = /private/CadenceVotingTokenMinter
